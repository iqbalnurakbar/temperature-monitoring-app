  ...
  try {
    const results = await Promise.all(
      apiConfigurations.map(async (config) => {
        ...
        const calculateStatistics = (fieldIndex, timeIndex) => {
          ...
          const minValue =
            validValues.length > 0 ? Math.min(...validValues) : NaN;
          const maxValue =
            validValues.length > 0 ? Math.max(...validValues) : NaN;
          ...
          const currentValue =
            values.length > 0 ? values[values.length - 1] : NaN;
        ...
        return config.fieldIndices.map((fieldIndex, index) =>
          calculateStatistics(fieldIndex, config.timeIndices[index]),
        );
      }),
    );
